...
(function () {
  ...
  /* Regional pricing config — VEXDYN-defined prices, not live FX conversion */
  const REGIONS = {
    NG: { name: "Nigeria", code: "+234", currency: "NGN", symbol: "₦" },
    US: { name: "United States", code: "+1", currency: "USD", symbol: "$" },
    GB: { name: "United Kingdom", code: "+44", currency: "GBP", symbol: "£" },
    CA: { name: "Canada", code: "+1", currency: "USD", symbol: "$" },
    GH: { name: "Ghana", code: "+233", currency: "USD", symbol: "$" },
    ZA: { name: "South Africa", code: "+27", currency: "USD", symbol: "$" },
    KE: { name: "Kenya", code: "+254", currency: "USD", symbol: "$" },
    DE: { name: "Germany", code: "+49", currency: "EUR", symbol: "€" },
    FR: { name: "France", code: "+33", currency: "EUR", symbol: "€" },
    OTHER: { name: "Other", code: "+", currency: "USD", symbol: "$" }
  };

  const BUDGETS_BY_CURRENCY = {
    NGN: ["Not sure yet", "Under ₦150,000", "₦150,000 – ₦350,000", "₦350,000 – ₦750,000", "₦750,000+", "Custom / Discuss"],
    USD: ["Not sure yet", "Under $300", "$300 – $800", "$800 – $2,000", "$2,000+", "Custom / Discuss"],
    GBP: ["Not sure yet", "Under £250", "£250 – £650", "£650 – £1,500", "£1,500+", "Custom / Discuss"],
    EUR: ["Not sure yet", "Under €300", "€300 – €750", "€750 – €1,800", "€1,800+", "Custom / Discuss"]
  };

  function getBudgets() {
    const region = REGIONS[state.country] || REGIONS.NG;
    return BUDGETS_BY_CURRENCY[region.currency] || BUDGETS_BY_CURRENCY.NGN;
  }

  const state = {
    buildMode: null,
    selectedWebsiteTypes: [],
    client: { name: "", brand: "", email: "", phone: "", localPhone: "" },
    country: "NG",
    countryCode: "+234",
    currency: "NGN",
    vision: "",
  ...
  function goStep(n) {
    ...
    if (n === 4) {
      ...
      fillChips("bfBudget", getBudgets(), false, state.budget);
    }
    if (n === 5) renderReview();
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  function validateStep(n) {
    if (n === 2) {
      ...
      if (ok) {
        ...
        var localPhone = (($("#bfPhone") || {}).value || "").trim().replace(/^\+/, "");
        state.client.localPhone = localPhone;
        state.client.phone = localPhone
          ? (state.countryCode + " " + localPhone).trim()
          : "";
  ...
  function init() {
    ...
    fillChips("bfBudget", getBudgets(), false, "");
    ...
    // Country → calling code + regional budget labels
    var countrySel = document.getElementById("bfCountry");
    var codeEl = document.getElementById("bfPhoneCode");
    if (countrySel) {
      countrySel.addEventListener("change", function () {
        var key = countrySel.value || "NG";
        var region = REGIONS[key] || REGIONS.NG;
        state.country = key;
        state.countryCode = region.code;
  ...
  window.VEXDYN_BUILD = { state: state, REGIONS: REGIONS };
})();
